...
//    printf("Should render incoming video stream");
...
//    printf("Got to end of IncomingVideoStream::RenderFrame");
...
int32_t IncomingVideoStream::SetRenderCallback(
  ...
  WEBRTC_TRACE(kTraceInfo, kTraceVideoRenderer, module_id_,
               ...
               stream_id_);
  render_callback_ = render_callback; // Look at the address when we set it.
                                    // Is this the same incoming video stream that we see frames coming through on?
  return 0;
}
