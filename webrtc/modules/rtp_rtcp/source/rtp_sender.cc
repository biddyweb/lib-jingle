...
//    printf("RTPSender::SendOutgoingData\n");
...
bool RTPSender::SendPacketToNetwork(const uint8_t *packet, uint32_t size) {
  int bytes_sent = -1;
//    printf("RTPSender::SendPacketToNetwork\n");
  if (transport_) {
//      printf("Transport - send packet\n");
    bytes_sent = transport_->SendPacket(id_, packet, size);
      // TODO - Figure out what this calls
  }
...
//      printf("Transport failed to send packet\n");
    return false;
  }
  return true;
}
...
//    printf("Attempting RTPSender::SendToNetwork\n");
...
//    printf("Should actuallu send packet to network\n");
