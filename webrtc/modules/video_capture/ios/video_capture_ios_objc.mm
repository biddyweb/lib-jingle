...
- (id)initWithOwner:(VideoCaptureIos*)owner captureId:(int)captureId {
      ...
      NSLog(@"Init VideoCaptureIosObjC");
  ...
  if (_captureSession) {
    ...
    if ([currentInputs count] > 0) {
      AVCaptureDeviceInput* currentInput = [currentInputs objectAtIndex:0];
        NSLog(@"Camera name: %@", [currentInput.device localizedName]);
...
- (BOOL)startCaptureWithCapability:(const VideoCaptureCapability&)capability {
      ...
      NSLog(@"Should start capture with capability");
  ...
  if ([_captureSession canSetSessionPreset:AVCaptureSessionPreset1920x1080]) {
    ...
    NSLog(@"Within limits of resolution");
  ...
  if ([currentOutputs count] == 0) {
    ...
    NSLog(@"There are current outputs");
  ...
  if (_capability.width >= 1920 || _capability.height >= 1080) {
    ...
    NSLog(@"Capture quality shall be: %@", captureQuality);
  ...
  [_captureSession startRunning];
    ...
    NSLog(@"Started running capture session");

  [captureQuality release];

  NSLog(@"Did start capture");
...
- (BOOL)changeCaptureInputByUniqueId:(NSString*)uniqueId {
    ...
    NSLog(@"In changeCaptureInputByUniqueId");
  ...
  if ([currentInputs count] > 0) {
    ...
    NSLog(@"Any current inputs removed");
  ...
  if (captureDeviceCount <= 0) {
      NSLog(@"There are no capture devices available");
    return NO;
  }
  ...
  if (!captureDevice) {
      NSLog(@"No capture device found for id");
    return NO;
  }

    NSLog(@"I think it works past here");
  ...
  [_captureSession commitConfiguration];
    ...
    NSLog(@"Added capture input: %i", addedCaptureInput);
